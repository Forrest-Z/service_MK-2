...
#define WHEEL_SEPARATION                  0.362          // meter

#ifdef BLDC_MOTOR
#define GEARRATIO                           (26)
#define ENCODER4CH                          (1024*4.0)
#else
#define GEARRATIO                           (119.368)
#define ENCODER4CH                          (13*4.0)
#endif
...
#define CONTROL_MOTOR_SPEED_PERIOD       (1000/25)     //50hz. 20ms
...
//#else
    ss << "SV" << -1*leftSpeed << "," << -1*rightSpeed << ";"<< "\0";    // None feedback
//#endif
...
//#ifdef DEBUG_MC        
        ROS_INFO_STREAM("Set Speed (" << "," << ")  " << ss.str());
//#endif
    }
}
...
int receive_motor_data()
{
    ...
    if(serial_comm.available()) {
        ...
        if(MCDataCnt>= 20) {
...
#if 1
                left_motor_vel = rpm2MPS(left_motor_vel_str, left_motor_dir)*-1;
                right_motor_vel = rpm2MPS(right_motor_vel_str, right_motor_dir)*-1;
            ...
            ROS_INFO_STREAM("ss.str() (" << revData <<  ")");
            return -2;
        }
